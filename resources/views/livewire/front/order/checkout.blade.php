...
<livewire:front.layout.menu :lang="$multiLanguage">
    ...
    <div class="checkout-area ptb-100">
            <div class="container">
                @if($order)
                <form role="form" action="{{route('front.checkout.redirect',$order->id)}}" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ env('STRIPE_KEY') }}" id="payment-form">
                    @csrf
                    <div class="row">
                            ...
                            <div class="billing-details">
                                ...
                                <div class="order-table table-responsive">
                                    <table class="table table-bordered">
                                        <tbody>
                                            <tr>
                                                <td class="product-name"><a href="products-details.html">{{$this->getTranslate('title',$pack)}}</a></td>
                                                <td class="product-total">
                                                    <span class="subtotal-amount">${{$pack->price}}</span>
                                                </td>
                                            </tr>
                                           
                                            <tr>
                                                <td class="total-price"><span>{{__('Total')}}</span></td>
